...
def isSameTree(p, q):
    if not p and not q:  #for the final step in the recursion when there are no more leaves return true
        return True
    if p and q and p.val == q.val:
        return isSameTree(p.left, q.left) and isSameTree(p.right, q.right) #checking for each node if left nodes and right nodes are same
    return False
